...
import com.aerothinker.plandb.service.ParaAttrService;
...
import com.aerothinker.plandb.service.ParaAttrQueryService;
...
public class ParaAttrResourceIntTest {
     ...
     *
     * @see com.aerothinker.plandb.repository.search.ParaAttrSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createParaAttr() throws Exception {
        ...
        assertThat(testParaAttr.getInsertTime()).isEqualTo(DEFAULT_INSERT_TIME);
        assertThat(testParaAttr.getUpdateTime()).isEqualTo(DEFAULT_UPDATE_TIME);
    ...
    @Test
    @Transactional
    public void getAllParaAttrsByInsertTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        paraAttrRepository.saveAndFlush(paraAttr);

        // Get all the paraAttrList where insertTime equals to DEFAULT_INSERT_TIME
        defaultParaAttrShouldBeFound("insertTime.equals=" + DEFAULT_INSERT_TIME);

        // Get all the paraAttrList where insertTime equals to UPDATED_INSERT_TIME
        defaultParaAttrShouldNotBeFound("insertTime.equals=" + UPDATED_INSERT_TIME);
    }

    @Test
    @Transactional
    public void getAllParaAttrsByInsertTimeIsInShouldWork() throws Exception {
        // Initialize the database
        paraAttrRepository.saveAndFlush(paraAttr);

        // Get all the paraAttrList where insertTime in DEFAULT_INSERT_TIME or UPDATED_INSERT_TIME
        defaultParaAttrShouldBeFound("insertTime.in=" + DEFAULT_INSERT_TIME + "," + UPDATED_INSERT_TIME);

        // Get all the paraAttrList where insertTime equals to UPDATED_INSERT_TIME
        defaultParaAttrShouldNotBeFound("insertTime.in=" + UPDATED_INSERT_TIME);
    }

    @Test
    @Transactional
    public void getAllParaAttrsByInsertTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        paraAttrRepository.saveAndFlush(paraAttr);

        // Get all the paraAttrList where insertTime is not null
        defaultParaAttrShouldBeFound("insertTime.specified=true");

        // Get all the paraAttrList where insertTime is null
        defaultParaAttrShouldNotBeFound("insertTime.specified=false");
    }

    @Test
    @Transactional
    public void getAllParaAttrsByUpdateTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        paraAttrRepository.saveAndFlush(paraAttr);

        // Get all the paraAttrList where updateTime equals to DEFAULT_UPDATE_TIME
        defaultParaAttrShouldBeFound("updateTime.equals=" + DEFAULT_UPDATE_TIME);

        // Get all the paraAttrList where updateTime equals to UPDATED_UPDATE_TIME
        defaultParaAttrShouldNotBeFound("updateTime.equals=" + UPDATED_UPDATE_TIME);
    }

    @Test
    @Transactional
    public void getAllParaAttrsByUpdateTimeIsInShouldWork() throws Exception {
        // Initialize the database
        paraAttrRepository.saveAndFlush(paraAttr);

        // Get all the paraAttrList where updateTime in DEFAULT_UPDATE_TIME or UPDATED_UPDATE_TIME
        defaultParaAttrShouldBeFound("updateTime.in=" + DEFAULT_UPDATE_TIME + "," + UPDATED_UPDATE_TIME);

        // Get all the paraAttrList where updateTime equals to UPDATED_UPDATE_TIME
        defaultParaAttrShouldNotBeFound("updateTime.in=" + UPDATED_UPDATE_TIME);
    }

    @Test
    @Transactional
    public void getAllParaAttrsByUpdateTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        paraAttrRepository.saveAndFlush(paraAttr);

        // Get all the paraAttrList where updateTime is not null
        defaultParaAttrShouldBeFound("updateTime.specified=true");

        // Get all the paraAttrList where updateTime is null
        defaultParaAttrShouldNotBeFound("updateTime.specified=false");
    }
    ...
    @Test
    @Transactional
    public void getAllParaAttrsByCreatedByIsEqualToSomething() throws Exception {
        // Initialize the database
        RmsUser createdBy = RmsUserResourceIntTest.createEntity(em);
        em.persist(createdBy);
        em.flush();
        paraAttr.setCreatedBy(createdBy);
        paraAttrRepository.saveAndFlush(paraAttr);
        Long createdById = createdBy.getId();

        // Get all the paraAttrList where createdBy equals to createdById
        defaultParaAttrShouldBeFound("createdById.equals=" + createdById);

        // Get all the paraAttrList where createdBy equals to createdById + 1
        defaultParaAttrShouldNotBeFound("createdById.equals=" + (createdById + 1));
    }


    @Test
    @Transactional
    public void getAllParaAttrsByModifiedByIsEqualToSomething() throws Exception {
        // Initialize the database
        RmsUser modifiedBy = RmsUserResourceIntTest.createEntity(em);
        em.persist(modifiedBy);
        em.flush();
        paraAttr.setModifiedBy(modifiedBy);
        paraAttrRepository.saveAndFlush(paraAttr);
        Long modifiedById = modifiedBy.getId();

        // Get all the paraAttrList where modifiedBy equals to modifiedById
        defaultParaAttrShouldBeFound("modifiedById.equals=" + modifiedById);

        // Get all the paraAttrList where modifiedBy equals to modifiedById + 1
        defaultParaAttrShouldNotBeFound("modifiedById.equals=" + (modifiedById + 1));
    }


    @Test
    @Transactional
    public void getAllParaAttrsByVerifiedByIsEqualToSomething() throws Exception {
        // Initialize the database
        RmsUser verifiedBy = RmsUserResourceIntTest.createEntity(em);
        em.persist(verifiedBy);
        em.flush();
        paraAttr.setVerifiedBy(verifiedBy);
        paraAttrRepository.saveAndFlush(paraAttr);
        Long verifiedById = verifiedBy.getId();

        // Get all the paraAttrList where verifiedBy equals to verifiedById
        defaultParaAttrShouldBeFound("verifiedById.equals=" + verifiedById);

        // Get all the paraAttrList where verifiedBy equals to verifiedById + 1
        defaultParaAttrShouldNotBeFound("verifiedById.equals=" + (verifiedById + 1));
    }
    ...
    public void updateParaAttr() throws Exception {
        ...
        assertThat(testParaAttr.getInsertTime()).isEqualTo(UPDATED_INSERT_TIME);
        assertThat(testParaAttr.getUpdateTime()).isEqualTo(UPDATED_UPDATE_TIME);
