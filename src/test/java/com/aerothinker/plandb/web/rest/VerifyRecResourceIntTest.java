...
public class VerifyRecResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllVerifyRecsByCreatedByIsEqualToSomething() throws Exception {
        // Initialize the database
        RmsUser createdBy = RmsUserResourceIntTest.createEntity(em);
        em.persist(createdBy);
        em.flush();
        verifyRec.setCreatedBy(createdBy);
        verifyRecRepository.saveAndFlush(verifyRec);
        Long createdById = createdBy.getId();

        // Get all the verifyRecList where createdBy equals to createdById
        defaultVerifyRecShouldBeFound("createdById.equals=" + createdById);

        // Get all the verifyRecList where createdBy equals to createdById + 1
        defaultVerifyRecShouldNotBeFound("createdById.equals=" + (createdById + 1));
    }
