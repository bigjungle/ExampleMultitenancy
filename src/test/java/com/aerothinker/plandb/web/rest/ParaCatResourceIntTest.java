...
import com.aerothinker.plandb.service.ParaCatService;
...
import com.aerothinker.plandb.service.ParaCatQueryService;
...
public class ParaCatResourceIntTest {
     ...
     *
     * @see com.aerothinker.plandb.repository.search.ParaCatSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createParaCat() throws Exception {
        ...
        assertThat(testParaCat.getInsertTime()).isEqualTo(DEFAULT_INSERT_TIME);
        assertThat(testParaCat.getUpdateTime()).isEqualTo(DEFAULT_UPDATE_TIME);
    ...
    @Test
    @Transactional
    public void getAllParaCatsByInsertTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        paraCatRepository.saveAndFlush(paraCat);

        // Get all the paraCatList where insertTime equals to DEFAULT_INSERT_TIME
        defaultParaCatShouldBeFound("insertTime.equals=" + DEFAULT_INSERT_TIME);

        // Get all the paraCatList where insertTime equals to UPDATED_INSERT_TIME
        defaultParaCatShouldNotBeFound("insertTime.equals=" + UPDATED_INSERT_TIME);
    }

    @Test
    @Transactional
    public void getAllParaCatsByInsertTimeIsInShouldWork() throws Exception {
        // Initialize the database
        paraCatRepository.saveAndFlush(paraCat);

        // Get all the paraCatList where insertTime in DEFAULT_INSERT_TIME or UPDATED_INSERT_TIME
        defaultParaCatShouldBeFound("insertTime.in=" + DEFAULT_INSERT_TIME + "," + UPDATED_INSERT_TIME);

        // Get all the paraCatList where insertTime equals to UPDATED_INSERT_TIME
        defaultParaCatShouldNotBeFound("insertTime.in=" + UPDATED_INSERT_TIME);
    }

    @Test
    @Transactional
    public void getAllParaCatsByInsertTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        paraCatRepository.saveAndFlush(paraCat);

        // Get all the paraCatList where insertTime is not null
        defaultParaCatShouldBeFound("insertTime.specified=true");

        // Get all the paraCatList where insertTime is null
        defaultParaCatShouldNotBeFound("insertTime.specified=false");
    }

    @Test
    @Transactional
    public void getAllParaCatsByUpdateTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        paraCatRepository.saveAndFlush(paraCat);

        // Get all the paraCatList where updateTime equals to DEFAULT_UPDATE_TIME
        defaultParaCatShouldBeFound("updateTime.equals=" + DEFAULT_UPDATE_TIME);

        // Get all the paraCatList where updateTime equals to UPDATED_UPDATE_TIME
        defaultParaCatShouldNotBeFound("updateTime.equals=" + UPDATED_UPDATE_TIME);
    }

    @Test
    @Transactional
    public void getAllParaCatsByUpdateTimeIsInShouldWork() throws Exception {
        // Initialize the database
        paraCatRepository.saveAndFlush(paraCat);

        // Get all the paraCatList where updateTime in DEFAULT_UPDATE_TIME or UPDATED_UPDATE_TIME
        defaultParaCatShouldBeFound("updateTime.in=" + DEFAULT_UPDATE_TIME + "," + UPDATED_UPDATE_TIME);

        // Get all the paraCatList where updateTime equals to UPDATED_UPDATE_TIME
        defaultParaCatShouldNotBeFound("updateTime.in=" + UPDATED_UPDATE_TIME);
    }

    @Test
    @Transactional
    public void getAllParaCatsByUpdateTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        paraCatRepository.saveAndFlush(paraCat);

        // Get all the paraCatList where updateTime is not null
        defaultParaCatShouldBeFound("updateTime.specified=true");

        // Get all the paraCatList where updateTime is null
        defaultParaCatShouldNotBeFound("updateTime.specified=false");
    }
    ...
    @Test
    @Transactional
    public void getAllParaCatsByCreatedByIsEqualToSomething() throws Exception {
        // Initialize the database
        RmsUser createdBy = RmsUserResourceIntTest.createEntity(em);
        em.persist(createdBy);
        em.flush();
        paraCat.setCreatedBy(createdBy);
        paraCatRepository.saveAndFlush(paraCat);
        Long createdById = createdBy.getId();

        // Get all the paraCatList where createdBy equals to createdById
        defaultParaCatShouldBeFound("createdById.equals=" + createdById);

        // Get all the paraCatList where createdBy equals to createdById + 1
        defaultParaCatShouldNotBeFound("createdById.equals=" + (createdById + 1));
    }


    @Test
    @Transactional
    public void getAllParaCatsByModifiedByIsEqualToSomething() throws Exception {
        // Initialize the database
        RmsUser modifiedBy = RmsUserResourceIntTest.createEntity(em);
        em.persist(modifiedBy);
        em.flush();
        paraCat.setModifiedBy(modifiedBy);
        paraCatRepository.saveAndFlush(paraCat);
        Long modifiedById = modifiedBy.getId();

        // Get all the paraCatList where modifiedBy equals to modifiedById
        defaultParaCatShouldBeFound("modifiedById.equals=" + modifiedById);

        // Get all the paraCatList where modifiedBy equals to modifiedById + 1
        defaultParaCatShouldNotBeFound("modifiedById.equals=" + (modifiedById + 1));
    }


    @Test
    @Transactional
    public void getAllParaCatsByVerifiedByIsEqualToSomething() throws Exception {
        // Initialize the database
        RmsUser verifiedBy = RmsUserResourceIntTest.createEntity(em);
        em.persist(verifiedBy);
        em.flush();
        paraCat.setVerifiedBy(verifiedBy);
        paraCatRepository.saveAndFlush(paraCat);
        Long verifiedById = verifiedBy.getId();

        // Get all the paraCatList where verifiedBy equals to verifiedById
        defaultParaCatShouldBeFound("verifiedById.equals=" + verifiedById);

        // Get all the paraCatList where verifiedBy equals to verifiedById + 1
        defaultParaCatShouldNotBeFound("verifiedById.equals=" + (verifiedById + 1));
    }
    ...
    public void updateParaCat() throws Exception {
        ...
        assertThat(testParaCat.getInsertTime()).isEqualTo(UPDATED_INSERT_TIME);
        assertThat(testParaCat.getUpdateTime()).isEqualTo(UPDATED_UPDATE_TIME);
